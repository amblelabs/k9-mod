...
public class K9Entity extends TameableEntity implements Angerable, NamedScreenHandlerFactory, ImplementedInventory {
    ...
    protected void initGoals() {
        ...
        this.goalSelector.add(3, new TemptGoal(this, 1.1, Ingredient.ofItems(K9Items.K9_WRISTBAND), false));
    ...
    protected SoundEvent getAmbientSound() {
        if (this.hasAngerTime()) {
            return K9Sounds.K9_DEFENSE;
        }
        if (this.random.nextInt(3) == 0) {
            if (this.isTamed() && this.getHealth() < 10.0f) {
                return K9Sounds.K9_MASTER;
            }
    ...
    protected SoundEvent getHurtSound(DamageSource source) {
        return K9Sounds.K9_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return K9Sounds.K9_DIE;
    }
        ...
        if (!this.getWorld().isClient()) {
            ...
            for(int i = 27; i < 35; i++) {
                if (this.getItems().get(i).getItem() instanceof K9LithiumCellItem) {
                    ...
                    if (nbt != null) {
                        if(nbt.getInt(K9LithiumCellItem.BATTERY_KEY) > 0) {
                            amountOfCells += 1;
                        }
                        additiveValue = additiveValue + nbt.getInt(K9LithiumCellItem.BATTERY_KEY);
                    }
                ...
                if (this.random.nextInt(199) == 0) { // 1 in 200 chance of detracting battery percentage
                    if (this.getBatteryLevel() > 0) {
                        ...
                        for (int i = 34; i > minNotDeadIndex; i--) {
                            if (this.getItems().get(i).getItem() instanceof K9LithiumCellItem) {
                                NbtCompound nbt = this.getItems().get(i).getNbt();
                                if (nbt != null && nbt.getInt(K9LithiumCellItem.BATTERY_KEY) > 0) {
                                    // Process the cell
                                    nbt.putInt(K9LithiumCellItem.BATTERY_KEY, Math.max(nbt.getInt(K9LithiumCellItem.BATTERY_KEY) - 1, 0));
    ...
    public boolean canTransferTo(Inventory hopperInventory, int slot, ItemStack stack) {
        if(slot >= 27 && stack.getItem() == K9Items.K9_LITHIUM_CELL) return false;
        else return true;
    }
        ...
        if (!this.getWorld().isClient()) {
            ...
            for (ItemEntity entity : entities) {
                if (entity.isOnGround()) {
                    ...
                    int getInvSize = entity.getStack().getItem() != K9Items.K9_LITHIUM_CELL ? 27 : 35;
                    int getISize = entity.getStack().getItem() != K9Items.K9_LITHIUM_CELL ? 27 : 8;
    ...
    public boolean damage(DamageSource source, float amount) {
        ...
        if(this.isTamed()) {
            if (this.getHealth() <= 10.0f || this.getBatteryLevel() <= 0) {
                this.getWorld().playSound(null, this.getBlockPos(), K9Sounds.K9_DIE, SoundCategory.AMBIENT, 7.0f, 1.0f);
            }
        }
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        if (this.getWorld().isClient()) {
            boolean bl = this.isOwner(player) || this.isTamed() || itemStack.isOf(K9Items.K9_BONE) && !this.isTamed() && !this.hasAngerTime();
        ...
        if (!itemStack.isOf(K9Items.K9_BONE) || this.hasAngerTime()) return super.interactMob(player, hand);
        if (!player.getAbilities().creativeMode) {
            itemStack.decrement(1);
        }
        if (this.random.nextInt(3) == 0) {
    ...
    public K9Entity createChild(ServerWorld serverWorld, PassiveEntity passiveEntity) {
        UUID uUID;
        K9Entity k9Entity = K9EntityTypes.K9_ENTITY_TYPE.create(serverWorld);
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new K9ScreenHandler(K9ScreenHandlers.K9_INVENTORY_HANDLER, syncId, playerInventory, this, properties);
    }

    public ScreenHandler createCellScreen(int syncId, PlayerInventory playerInventory) {
        return new K9CellScreenHandler(K9ScreenHandlers.K9_CELL_HANDLER, syncId, playerInventory, this, properties);
    }
