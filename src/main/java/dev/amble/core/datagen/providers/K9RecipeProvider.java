...
public class K9RecipeProvider extends FabricRecipeProvider {
    ...
    public K9RecipeProvider(FabricDataOutput output) {
        super(output);
    }
    ...
    private Identifier getStonecuttingIdentifier(Block in, Block out) {
        return K9Mod.id(fixupBlockKey(in.getTranslationKey()) + "_to_" + fixupBlockKey(out.getTranslationKey()) + "_stonecutting");
    }
